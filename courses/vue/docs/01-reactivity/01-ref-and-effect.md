...
```text
apps/learning-portal/src/pages/vue
    ↓ 只从 mini-vue 导入公开 API
courses/vue/packages/vue
    ↓ 统一导出各个核心模块
courses/vue/packages/reactivity
    ↓ 使用公共工具
courses/vue/packages/shared
```

- `courses/vue/packages/shared`：所有模块都可能使用的小工具。
- `courses/vue/packages/reactivity`：你当前学习的 `ref`、`effect`，以后还会有 `reactive`、`computed`。
- `courses/vue/packages/vue`：对使用者暴露的统一入口。以后即使内部多出许多包，学习门户中的导入方式也不用变化。
- `apps/learning-portal/src/pages/vue`：Vue 浏览器实验页。这里不是框架源码，只负责调用你写的 mini-vue。
...
先打开 `apps/learning-portal/src/main.ts`，不要急着修改。按实际执行顺序观察：
...
先只修改 `courses/vue/packages/reactivity/src/effect.ts`。这一小段属于必要的脚手架，可以照着写，然后逐行解释给自己听：
...
接着修改 `courses/vue/packages/reactivity/src/ref.ts`。先搭出下面的结构，TODO 部分暂时留空：
...
项目的 `courses/vue/packages/shared/src/index.ts` 已经提供了 `hasChanged`。它内部使用 `Object.is`，所以你也可以导入它，让判断读起来更接近自然语言：只有 `hasChanged(newValue, this._value)` 为真时才继续更新。
