...
| 响应式 | [21. watch 数据源标准化与多数据源](./01-reactivity/21-watch-sources.md) | 已批改 |

第 20 章的调用过程、依赖选择、键身份、迭代协议和 deep watch 衔接共 5 题已作答并批改。第 21 章的 source 标准化、身份比较、数组歧义和类型推导共 5 题也已完成批改。
...
- 理论问题不适合时，可以使用编程题或理论与编程混合题；编程题通常聚焦 1～3 个函数或约 10～40 行核心代码。
- 编程题优先采用新场景、补全关键逻辑、代码审查、调试或小范围重构，避免只有一两行，也避免扩张成新的完整章节。
