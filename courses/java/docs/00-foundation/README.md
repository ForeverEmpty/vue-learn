...
这个目录独立于 Java 各个正式模块，用来补充学习过程中实际遇到的语言基础。并发、反射、泛型、I/O 或 JVM 模块出现前置知识障碍时，可以暂时进入对应小章；理解后再返回原检查点。
...
| 09 | [注解与元注解](./09-annotations-and-meta-annotations.md) | 按需学习 | 区分注解语法与元注解，为反射读取运行时元数据及 Spring 注解机制打基础。 |
